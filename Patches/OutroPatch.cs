        ...
        public static void Postfix(EndGameManager __instance)
        {
            ...
            if (PlayerControl.LocalPlayer.PlayerId == 0 && Main.SendResultToDiscord.Value)
            {
                ...
                else
                {
                    ...
                    foreach (var id in Main.winnerList)
                    {
                        resultMessage += Utils.ColorIdToDiscordEmoji(Palette.PlayerColors.IndexOf(Main.PlayerColors[id]), !Main.PlayerStates[id].IsDead) + ":star:" + EndGamePatch.SummaryText[id].RemoveHtmlTags() + "\n";
                    }
                    foreach (var id in cloneRoles)
                    {
                        resultMessage += Utils.ColorIdToDiscordEmoji(Palette.PlayerColors.IndexOf(Main.PlayerColors[id]), !Main.PlayerStates[id].IsDead) + "\u3000" + EndGamePatch.SummaryText[id].RemoveHtmlTags() + "\n";
                    }
